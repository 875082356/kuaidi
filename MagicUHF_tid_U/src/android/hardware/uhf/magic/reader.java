...
public class reader {
	...
	static public int Writelables(byte[] password, int nUL, byte[] PCandEPC,
		...
		System.out.println("Writelables");
		Clean();
		int nret = WriteLable(password, nUL, PCandEPC, membank, nSA, nDL, data);
		System.out.println("m_bASYC"+m_bASYC);
		//	m_bASYC=false;
		if (!m_bASYC) {
			m_bOK = false;
			m_nReSend = 0;
	...
	static void StartASYCWritelables() {
		System.out.println("StartASYCWritelables");
		m_bASYC = true;
		Thread thread = new Thread(new Runnable() {
			public void run() {
				int nTemp = 0;
						...
						// for (int i = 0; i < substr.length; i++)
						{
							if (str.length() >= 12) {
								...
								if (str.substring(10, 12).equals("00")
									...
									System.out.println("读标签Error:该标签没有在场区或者指定的EPC 代码不对");
									m_handler.sendMessage(msg);
									break;
